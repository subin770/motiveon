		...
		<section class="content">
			<div class="card">
				...
				<div class="card-header with-border" style="text-align: right;">
					<h3
						style="text-align: left; padding-left: 20px; margin-top: 10px; font-size: 18px; font-weight: 600;">자료실</h3>
					<button type="button" class="btn btn-primary" id="registBtn"
						onclick="OpenWindow('regist','등록',700,800);">등 록</button>
					<button type="button" class="btn btn-danger"
						onclick="deleteSelected()">삭 제</button>
				</div>
				<div class="card-body">
						...
						<tbody>
							...
							<c:if test="${not empty popUpList}">
								<c:forEach items="${popUpList}" var="popup">
									<tr
										onclick="OpenWindow('detail?popNo=${popup.popNo}','자료 상세',700,800);"
										style="cursor: pointer;">
										<td
											onclick="toggleCheckbox(this, event); event.stopPropagation();"><input
											type="checkbox" name="popNos" value="${popup.popNo}"
    ...
    <c:when test="${not empty attachList}">
        <ul>
            <c:forEach var="file" items="${attachList}">
                <li><a href="${file.uploadPath}${file.fileName}" target="_blank">${file.fileName}</a></li>
            </c:forEach>
        </ul>
    </c:when>
    <c:otherwise>
