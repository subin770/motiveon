...
.stat.wait { background: #D6EFD8; }
.stat.prog { background: #F6D6C6; }
.stat.done { background: #FFF7AE; }
.stat.agree { background: #F4D4ED; }
.stat.dele { background: #D9D9D9; }
.stat.all { background: #E3E7EF; }
                        ...
                        <tbody id="tbody">
                            <c:forEach var="work" items="${reqList}">
                                <tr data-status="${work.wstatus}" 
                                    ...
                                    <td>
                                        <div class="t-title"
                                            onclick="openReqDetail('${work.wcode}')">
                                            ${work.wtitle}
                                        </div>
                                        <div class="t-sub">
                            ...
                            <c:if test="${empty reqList}">
                                <tr>
                                    <td colspan="4" class="text-center"
                                        style="padding: 40px 0; color: #95A1AF;">
                                        표시할 업무가 없습니다.
                                    </td>
                                </tr>
                            </c:if>
...
<script>
    // 요청한 업무 상세보기 팝업 오픈
    function openReqDetail(wcode){
        let url = '<%=ctx%>/work/workReqDetail?wcode=' + wcode;
        window.open(url, 'reqDetailPopup', 'width=900,height=700,scrollbars=yes,resizable=yes');
    }
    ...
    $('#btnSearch').on('click', function(){
      ...
      $('#tbody tr').each(function(){
        ...
        else if(f==='requester') target = ($(this).data('requester')||'').toLowerCase();
